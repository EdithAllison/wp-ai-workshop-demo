<?php
/**
 * Post generation and creation functions for WP AI Workshop Demo.
 *
 ...
 * @return array
 */
function wp_ai_workshop_demo_generate_post( $arguments ) {
	$content = wp_ai_workshop_generate_content( $arguments['prompt'] );
	if ( is_wp_error( $content ) ) {
		return array(
			'message' => 'Post creation failed: ' . $content->get_error_message(),
		);
	}
	$image   = wp_ai_workshop_demo_create_image( $arguments['title'] );
	if ( is_wp_error( $image ) ) {
		return array(
			'message' => 'Post creation failed: ' . $image->get_error_message(),
		);
	}
    return wp_ai_workshop_demo_create_post( $arguments['title'], $content, $image );
}
 ...
 * @return array
 */
function wp_ai_workshop_demo_create_post( $title, $content, $image ) {
    ...
    $attachment_id = wp_ai_workshop_demo_image_to_media( $image, 'featured-image-' . sanitize_file_name( $title ), $post_id );
